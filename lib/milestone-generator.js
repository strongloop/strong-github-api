  ...
  if (options.list) {
    ...
    return Promise.try(function() {
      ...
      return Promise.map(list.repos, function(repo) {
        // Retrieve all of the repositories first so that we can avoid
        // creating some of the milestones and failing mid-run.
        return Promise.try(function() {
          return octo.repos(repo.org, repo.name).fetch();
        }).catch(function(err) {
          return Promise.reject(
            new Error(fmt('unable to create milestone for %s/%s: %s',
            repo.org, repo.name, err.message)));
        });
      }).then(function(repos) {
